...
import { initR2Upload } from "@/lib/api";
import type { UploadMode } from "@/lib/uploadMode";
...
export function useUpload() {
  ...
  const start = useCallback(async (file: File, mode: UploadMode) => {
    ...
    try {
      if (mode === "r2") {
        ...
        await xhrPut(init.upload_url, file, (p) => {
        ...
        setState((s) => ({
          ...
          status: "done",
          progress: 1,
          result: { sceneId: init.scene_id, r2Key: init.r2_key },
        }));
      } else {
        ...
        setState((s) => ({
          ...
          status: "done",
          progress: 1,
          result: { sceneId: out.scene_id, modalPath: out.modal_path },
        }));
      }
    } catch (err) {
...
function xhrPostMultipart(
  ...
  onProgress: (p: number) => void,
  ref: React.MutableRefObject<XMLHttpRequest | null>,
): Promise<{ scene_id: string; modal_path: string }> {
  return new Promise((resolve, reject) => {
    const xhr = new XMLHttpRequest();
    ref.current = xhr;
    xhr.open("POST", url);
