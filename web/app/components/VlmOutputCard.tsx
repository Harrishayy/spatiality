...
function KeyframeWithBboxes({
    ...
    return (
      <div className="rounded border border-ink-800 bg-ink-900/40 p-3 text-[11px] text-ink-500">
        ...
        {process.env.NEXT_PUBLIC_MODAL_GET_ARTIFACT_URL
          ? "Modal endpoint may not yet allow frames/ paths — redeploy modal_app.py."
          : "frame URL not configured."}
      </div>
    );
  }
  ...
  return (
    <div className="relative overflow-hidden rounded border border-ink-800 bg-ink-950">
      {/* Use a plain img — Next/Image would need to be configured per-host
          for Modal/R2, and we don't get any optimisation benefit because
          these images are already JPEG-quality-compressed by ffmpeg. */}
