...
capture-real scene_id="demo_scene_v1" video="samples/capture.mp4" fps="2.0":
...
# Real VGGT + 3DGRUT inference (requires bundled deps + weights)
infer-real scene_id="demo_scene_v1" iterations="7000":
    uv run python -m inference --scene-id {{scene_id}} --real --iterations {{iterations}}
...
modal-inference scene_id="demo_scene_v1" iterations="7000":
    @test -n "$MODAL_INFERENCE_URL" || (echo "MODAL_INFERENCE_URL not set"; exit 1)
    curl -fsS -X POST "$MODAL_INFERENCE_URL" \
      -H 'content-type: application/json' \
      -d '{"scene_id": "{{scene_id}}", "iterations": {{iterations}}}'

modal-segment scene_id="demo_scene_v1" keyframes="5":
