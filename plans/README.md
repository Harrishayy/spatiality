...
Decentralized plan set for the Unicorn Mafia AI hackathon (24h, solo + Claude). Each module has a self-contained plan with file contracts; modules don't depend on each other at runtime, only on shared schemas.
...
7. [`modules/06_web.md`](./modules/06_web.md) — splat viewer + agent UI
8. [`modules/07_agent.md`](./modules/07_agent.md) — chat backend
...
## Owners
- **Harrish + Claude Code:** all modules — capture, inference, segmentation, object-isolation, web, agent, observability, deployment glue.
